...
import numpy as np
DEFAULT_SAMPLE_RANGE=[10000,50000,80000,100000,200000,500000,700000,1000000,3000000,5000000,7000000,100000000]
...
def run_benchmarks():
    ...
    print results
    return
def algorithm_benchmarks(event_file,scatter_file=None,algorithm=None,plot=False,save_mat=True, mat_version='7.3',
    sample_range=DEFAULT_SAMPLE_RANGE,iteration_size_range=DEFAULT_ITERATION_RANGE,dc=True,mt=True,repeat_times=1):
    ...
    for nsamples in sample_range:
        ...
        if mt:
            test_inversion=Inversion(data_file=event_file,angle_scatter_file_path=scatter_file,
                algorithm=algorithm,parallel=False,dc=False,max_samples=nsamples,chain_length=nsamples,output_format=False)
            ...
            if not results['samples'].__contains__(nsamples):
                results['samples'][nsamples]={}
            results['samples'][nsamples]['MT']=times
        if dc:
            test_inversion=Inversion(data_file=event_file,angle_scatter_file_path=scatter_file,
                algorithm=algorithm,parallel=False,dc=True,max_samples=nsamples,chain_length=nsamples,output_format=False)
            test_inversion.pool=False
            test_inversion.number_samples=min(test_inversion.number_samples,nsamples)
            times,nsamples=_full_inversion_time(test_inversion,repeat_times)
            if not results['samples'].__contains__(nsamples):
...
def _iteration_step_time(test_inversion,repeat_times):
    ...
    while i<repeat_times:
        if test_inversion._algorithm_name.lower()=='iterate':
            a_polarity,error_polarity,a1_amplitude_ratio,a2_amplitude_ratio,amplitude_ratio,error1_amplitude_ratio,error2_amplitude_ratio,a_polarity_prob,polarity_prob,error_polarity_prob=test_inversion._station_angles(test_inversion.data[0],0)
            starttime=default_timer()
            MTs,end=test_inversion._parse_job_result(False)
            result=ForwardTask(MTs,a_polarity,error_polarity,a1_amplitude_ratio,a2_amplitude_ratio,amplitude_ratio,error1_amplitude_ratio,error2_amplitude_ratio,a_polarity_prob,polarity_prob,error_polarity_prob,return_zero=True,reuse=False).__call__()
...
def _write_default_event():
    ...
    cPickle.dump(_default_event(),open(data_file,'wb'))
    return data_file
...
def probability_cython_benchmarks(n=1):
    ...
    Ap = np.matrix([[-0.0801, -0.5611, -0.3588, -0.2998, 0.2398, 0.6345],
    ...
    P = Ap*MT
    ErrorPolarity = np.matrix([[0.0010],
                               [0.0010],
                               [0.0010],
                               [0.0010],
                               [0.0010],
    ...
    for i in range(n):
        starttime = time.time()
        p = polarity_ln_pdf(P, ErrorPolarity)
        endtime = time.time()
    ...
    for i in range(n):
        starttime = time.time()
        p = polarity_ln_pdf(X, ErrorPolarity)
        endtime = time.time()
        print('\tNormal time '+str(endtime-starttime))
        times.append(endtime-starttime)
    print('\nNormal Avg ', sum(times)/n)
    ...
    _CYTHON = False
    Z = np.random.randn(Ap.shape[0], 1)
    psx = np.random.randn(Ap.shape[0], 1)
    psy = np.random.randn(Ap.shape[0], 1)
    print('\nAmplitude Ratio No Location samples NSamples=1000')
    times = []
    for i in range(n):
        starttime = time.time()
        p = amplitude_ratio_ln_pdf(
            Z, P, P, 100*ErrorPolarity, 100*ErrorPolarity)
        endtime = time.time()
        print('\tNormal time '+str(endtime-starttime))
        times.append(endtime-starttime)
    print('\nNormal Avg ', sum(times)/n)
    ...
    psx = np.random.randn(X.shape[0], 1)
    psy = np.random.randn(X.shape[0], 1)
    print('\nAmplitude Ratio Location 200 samples NSamples=50')
    times = []
    for i in range(n):
        starttime = time.time()
        p = amplitude_ratio_ln_pdf(
            Z, X, X, 100*ErrorPolarity, 100*ErrorPolarity)
        endtime = time.time()
        print('\tNormal time '+str(endtime-starttime))
        times.append(endtime-starttime)
    print('\nNormal Avg ', sum(times)/n)
